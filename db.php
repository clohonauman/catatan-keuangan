...
define('USER_DATA_DIR', DATA_DIR.'/user_finance');
define('LEGACY_DATA_FILE', DATA_DIR.'/finance.json');
...
function defaultDailyBudgetSettings() {
    return [
        ...
        'warning_percent'=>80,
        'days'=>[
            '1'=>['enabled'=>false,'limit'=>0],
            '2'=>['enabled'=>false,'limit'=>0],
            '3'=>['enabled'=>false,'limit'=>0],
            '4'=>['enabled'=>false,'limit'=>0],
            '5'=>['enabled'=>false,'limit'=>0],
            '6'=>['enabled'=>false,'limit'=>0],
            '7'=>['enabled'=>false,'limit'=>0]
        ]
    ];
}
...
function defaultData() {
    return [
        'settings'=>[
            'initial_balance'=>0,
            'daily_budget'=>defaultDailyBudgetSettings()
        ],
        'transactions'=>[],
        'chats'=>[],
        'meta'=>['next_transaction_id'=>1,'next_chat_id'=>1,'revision'=>0]
    ];
}

function currentUserDataFile() {
    $user = authCurrentUser();
    if (!$user) throw new RuntimeException('User belum login.');
    return USER_DATA_DIR.'/user_'.(int)$user['id'].'.json';
}

function userDataFileById($userId) {
    $userId = (int)$userId;
    if ($userId <= 0) throw new InvalidArgumentException('User tidak valid.');
    return USER_DATA_DIR.'/user_'.$userId.'.json';
}

function ensureUserDataById($userId) {
    if (!is_dir(DATA_DIR)) mkdir(DATA_DIR, 0775, true);
    $protect = DATA_DIR.'/.htaccess';
    if (!file_exists($protect)) @file_put_contents($protect, "Options -Indexes\n<IfModule mod_authz_core.c>\nRequire all denied\n</IfModule>\n<IfModule !mod_authz_core.c>\nDeny from all\n</IfModule>\n", LOCK_EX);
    if (!is_dir(USER_DATA_DIR)) mkdir(USER_DATA_DIR, 0775, true);
    $file = userDataFileById($userId);
    if (!file_exists($file)) {
        file_put_contents($file, json_encode(defaultData(), JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE), LOCK_EX);
    }
    return $file;
}

function mutateUserDataById($userId, $fn) {
    $file = ensureUserDataById($userId);
    $fp = fopen($file, 'c+');
    if (!$fp) throw new RuntimeException('Tidak dapat membuka data user.');
    flock($fp, LOCK_EX);
    rewind($fp);
    $raw = stream_get_contents($fp);
    $data = json_decode($raw, true);
    if (!is_array($data)) $data = defaultData();
    else $data = array_replace_recursive(defaultData(), $data);
    $result = call_user_func_array($fn, [&$data]);
    if (!isset($data['meta']) || !is_array($data['meta'])) $data['meta'] = [];
    $data['meta']['revision'] = max(0, (int)($data['meta']['revision'] ?? 0)) + 1;
    rewind($fp);
    ftruncate($fp, 0);
...
function addChatForUser($userId, $role, $message, $attachment=null) {
    $r = mutateUserDataById((int)$userId, function (&$d) use ($role,$message,$attachment) {
        if (!isset($d['meta']['next_chat_id'])) {
            $max = 0;
            foreach (($d['chats'] ?? []) as $c) $max = max($max, (int)($c['id'] ?? 0));
            $d['meta']['next_chat_id'] = $max + 1;
        }
        ...
        if (!isset($d['chats']) || !is_array($d['chats'])) $d['chats'] = [];
        $d['chats'][] = $x;
        return $x;
    });
    return $r['result'];
...
function ensureData() {
    if (!is_dir(DATA_DIR)) mkdir(DATA_DIR, 0775, true);
    // Lindungi file JSON bila server memakai Apache. Endpoint PHP tetap dapat membacanya.
    $protect = DATA_DIR.'/.htaccess';
    if (!file_exists($protect)) @file_put_contents($protect, "Options -Indexes\n<IfModule mod_authz_core.c>\nRequire all denied\n</IfModule>\n<IfModule !mod_authz_core.c>\nDeny from all\n</IfModule>\n", LOCK_EX);
    if (!is_dir(USER_DATA_DIR)) mkdir(USER_DATA_DIR, 0775, true);
    $file = currentUserDataFile();
    if (!file_exists($file)) {
        $seed = defaultData();
        // Migrasi data versi single-user ke akun pertama bila file lama berisi data.
        $user = authCurrentUser();
        if ((int)$user['id'] === 1 && file_exists(LEGACY_DATA_FILE)) {
            $legacyRaw = @file_get_contents(LEGACY_DATA_FILE);
            $legacy = json_decode((string)$legacyRaw, true);
            if (is_array($legacy)) $seed = array_replace_recursive(defaultData(), $legacy);
        }
...
function readData() {
    ...
    fclose($fp);
    $data = json_decode($raw, true);
    return is_array($data) ? array_replace_recursive(defaultData(), $data) : defaultData();
}
...
function mutateData($fn) {
    ...
    $file = currentUserDataFile();
    $fp = fopen($file, 'c+');
    if (!$fp) throw new RuntimeException('Tidak dapat membuka file data user. Periksa permission folder data.');
    flock($fp, LOCK_EX);
    rewind($fp);
    $raw = stream_get_contents($fp);
    $data = json_decode($raw, true);
    ...
    $result = call_user_func_array($fn, [&$data]);
    // Revision dipakai frontend untuk mendeteksi perubahan data dari device/tab lain
    // tanpa perlu refresh halaman penuh.
    if (!isset($data['meta']) || !is_array($data['meta'])) $data['meta'] = [];
    $data['meta']['revision'] = max(0, (int)($data['meta']['revision'] ?? 0)) + 1;
    rewind($fp);
    ftruncate($fp, 0);
...
function setting($key,$default=null){$d=readData();return isset($d['settings'][$key])?$d['settings'][$key]:$default;}

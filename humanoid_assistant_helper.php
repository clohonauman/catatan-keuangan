...
function humanoidResolveContext(string $message): string {
    ...
    $looksShortFollowup = (bool)preg_match('/^(?:kalau|yang|terus|lalu|kalau yang|untuk|di|dari)\b/u',$t) || $wallet || $category;
...
function humanoidAmountFromText(string $message): int {
    $t = norm($message);
    $matches=[];
    preg_match_all('/(?:rp\s*)?\d[\d\.,]*(?:\s*(?:juta|jt|ribu|rb|k))?/iu',$t,$matches);
    $best=0;
    foreach (($matches[0]??[]) as $raw) {
        $amount=parseAmount((string)$raw);
        // Plain one/two digit numbers are usually dates/counts, not rupiah.
        $hasUnit=(bool)preg_match('/rp|juta|jt|ribu|rb|k/iu',(string)$raw);
...
function humanoidEditPendingConfirmation(string $message,array $pending): ?array {
    $t=norm($message);
    if(!preg_match('/\b(?:ubah|ganti|koreksi|salah|harusnya|seharusnya|jadikan|pakai|gunakan|dari|tanggal|kategori|catatan|keterangan|nominal|harian|sekali|berulang|rutin)\b/u',$t))return null;
    ...
    $changes=[];
    $amount=0;
    if(preg_match('/\b(?:ubah|ganti|koreksi|nominal|harusnya|seharusnya|jadi)\b/u',$t))$amount=humanoidAmountFromText($message);
    $wallet=walletMentionForText($message);
    ...
    if($amount<=0&&!$wallet&&$date===''&&$kind===''&&$category===''&&$note===null)return null;
    $id=(int)($pending['id']??0);
    $r=financeMutate(function(&$d)use($id,$indexes,$amount,$wallet,$date,$kind,$category,$note,&$changes){
        ...
        foreach($indexes as $idx){
            ...
            if($wallet){$draft['wallet_id']=(int)$wallet['id'];$changes['dompet']=(string)$wallet['name'];}
            if($date!==''){$draft['transaction_date']=$date;$changes['tanggal']=date('d/m/Y',strtotime($date));}
            if($kind!==''){$draft['spending_kind']=$kind;$changes['pola']=['daily'=>'Harian','once'=>'Sekali bayar','recurring'=>'Berulang'][$kind]??$kind;}

...
if(!defined('ADMIN_NOTIFICATION_FILE')) define('ADMIN_NOTIFICATION_FILE','mysql://admin_notifications');
...
function adminNotificationEnsureData(): void { if(!DocumentRepository::get('admin','notifications',[])) DocumentRepository::put('admin','notifications',adminNotificationDefaultData()); }

function adminNotificationReadData(): array { adminNotificationEnsureData(); $data=DocumentRepository::get('admin','notifications',adminNotificationDefaultData()); if(!isset($data['notifications'])||!is_array($data['notifications']))$data['notifications']=[];if(!isset($data['meta'])||!is_array($data['meta']))$data['meta']=['next_id'=>1];$max=0;foreach($data['notifications'] as $n)$max=max($max,(int)($n['id']??0));$data['meta']['next_id']=max((int)($data['meta']['next_id']??1),$max+1);return $data; }

function adminNotificationMutateData(callable $fn){ return DocumentRepository::mutate('admin','notifications',adminNotificationDefaultData(),function(&$data)use($fn){if(!isset($data['notifications'])||!is_array($data['notifications']))$data['notifications']=[];if(!isset($data['meta'])||!is_array($data['meta']))$data['meta']=['next_id'=>1];return $fn($data);}); }
...
function adminNotificationEmailLogPath(): string { return 'mysql://email_delivery_log'; }

function adminNotificationLogEmail(array $row): void { DocumentRepository::mutate('email','delivery_log',['emails'=>[]],function(&$d)use($row){if(!isset($d['emails'])||!is_array($d['emails']))$d['emails']=[];$d['emails'][]=$row;if(count($d['emails'])>2000)$d['emails']=array_slice($d['emails'],-2000);}); }

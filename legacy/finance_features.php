...
function financeConfirmPendingChat($confirmationId,array $overrides=[]){
    ...
    foreach((array)$pending['drafts'] as $i=>$draft){
        ...
        if($type==='transfer'){
            $from=(int)($draft['from_wallet_id']??$draft['wallet_id']??financeDefaultWalletId());
            $to=(int)($draft['to_wallet_id']??0);
            if($from<1||$to<1||$from===$to)throw new InvalidArgumentException('Transfer harus memakai dua dompet yang berbeda.');
            if(!financeWalletById($from)||!financeWalletById($to))throw new InvalidArgumentException('Dompet transfer tidak ditemukan.');
            $draft['from_wallet_id']=$from;$draft['to_wallet_id']=$to;
            $draft['category']='Transfer Antar Dompet';$draft['spending_kind']='once';$draft['bill_id']=0;
            unset($draft['wallet_id']);
        } else {
            $draft['wallet_id']=(int)($draft['wallet_id']??$draft['from_wallet_id']??financeDefaultWalletId());
            if(!financeWalletById((int)$draft['wallet_id']))throw new InvalidArgumentException('Dompet transaksi tidak ditemukan.');
            unset($draft['from_wallet_id'],$draft['to_wallet_id']);
            $draft['spending_kind']=$type==='expense'
                ? (in_array(($draft['spending_kind']??''),['daily','once','recurring'],true)?$draft['spending_kind']:transactionSpendingKind($draft))
                : 'once';
            if($type==='income')$draft['bill_id']=0;
        }
        $billId=(int)($draft['bill_id']??0);unset($draft['bill_candidates']);
        if($type==='expense'&&$billId>0){

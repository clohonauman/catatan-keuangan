(() => {
  ...
  function buildHeaders(record) {
    ...
    // CSRF token bisa berubah setelah login ulang; selalu gunakan token halaman aktif saat replay.
    const currentCsrf = document.querySelector('meta[name="csrf-token"]')?.content || '';
    if (currentCsrf) h.set('X-CSRF-Token', currentCsrf);
  ...
  function parseApiJsonPayload(raw) {
    ...
    for (let i = 0; i < clean.length; i++) {
      ...
      if (ch === '}' || ch === ']') {
        ...
        if (!open || (open === '{' && ch !== '}') || (open === '[' && ch !== ']')) return null;
        if (!stack.length) {
          try { return JSON.parse(clean.slice(0, i + 1)); } catch (_) { return null; }
        }
      }
    }
    return null;
  ...
  async function syncQueue() {
    ...
    try {
      ...
      for (const row of rows) {
        const routedUrl = routeLegacyUrl(row.url);
        const target = /^https?:\/\//i.test(routedUrl) ? routedUrl : currentSyncBase() + '/' + String(routedUrl).replace(/^\//, '');
        try {
          ...
          const json = parseApiJsonPayload(raw);

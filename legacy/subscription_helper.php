...
if (!defined('SUBSCRIPTION_FILE')) define('SUBSCRIPTION_FILE','mysql://subscription');
if (!defined('PAYMENT_PROOF_DIR')) define('PAYMENT_PROOF_DIR', Yii::$app->params['privateStorage'].'/payment_proofs');
...
function subscriptionEnsureData(): void { if(!is_dir(PAYMENT_PROOF_DIR)) @mkdir(PAYMENT_PROOF_DIR,0775,true); $d=SubscriptionRepository::read(subscriptionDefaultData()); if(empty($d['plans'])||empty($d['banks'])) SubscriptionRepository::replace(subscriptionDefaultData()); }
...
function subscriptionReadData(): array { subscriptionEnsureData(); return subscriptionNormalizeData(SubscriptionRepository::read(subscriptionDefaultData())); }

function subscriptionMutateData(callable $fn) { subscriptionEnsureData(); return SubscriptionRepository::mutate(subscriptionDefaultData(), function(&$data) use($fn){ $data=subscriptionNormalizeData($data); return $fn($data); }); }
...
function subscriptionStartTrial(array $user, bool $confirmed): array {
    $userId = (int)($user['id'] ?? 0);
    if ($userId <= 0) throw new InvalidArgumentException('Akun tidak ditemukan.');
    if (authIsSuperAdmin($user)) throw new InvalidArgumentException('Super Admin sudah memiliki akses penuh dan tidak memerlukan Free Trial.');
    if (!$confirmed) throw new InvalidArgumentException('Konfirmasi diperlukan untuk mengaktifkan Free Trial Premium 7 hari.');

    $startAt = date('Y-m-d H:i:s');
    $expiresAt = (new DateTimeImmutable('now'))->modify('+7 days')->format('Y-m-d H:i:s');

    $updated = authMutateUserData($userId, function (&$data) use ($userId, $startAt, $expiresAt) {
        foreach ($data['users'] as &$u) {
            if ((int)($u['id'] ?? 0) !== $userId) continue;

            $trial = authTrialStatus($u);
            if (!empty($trial['active'])) return $u;
            if (empty($trial['eligible'])) {
                if (!empty($trial['used'])) throw new InvalidArgumentException('Free Trial Premium hanya dapat digunakan satu kali untuk setiap akun.');
                throw new InvalidArgumentException('Akun ini tidak memenuhi syarat untuk Free Trial Premium.');
            }

            $u['premium_trial_started_at'] = $startAt;
            $u['premium_trial_expires_at'] = $expiresAt;
            $u['premium_trial_confirmed_at'] = $startAt;
            $u['premium_trial_used_at'] = $startAt;
            unset($u['premium_trial_cancelled_at']);
            return $u;
        }
        unset($u);
    ...
    return is_array($updated) ? $updated : (authFindUserById($userId) ?: $user);
}
...
function subscriptionPlanExpiryForApproval(array $order, array $user): string {
    ...
    if (($existingPlan['source'] ?? '') === 'trial') {
        $trialExpiry = trim((string)($existingPlan['expires_at'] ?? ''));
        $trialExpiryTs = $trialExpiry !== '' ? strtotime($trialExpiry) : false;
        if ($trialExpiryTs !== false && $trialExpiryTs > time()) {
            $base = new DateTimeImmutable(date('Y-m-d H:i:s', $trialExpiryTs));
        }
    }
...
function subscriptionOrderForClient(?array $order): ?array {
    ...
    $out['proof_url'] = !empty($order['proof']['file']) ? 'index.php?r=legacy-api%2Fbridge&name=payment_proof&order_id='.(int)$order['id'] : '';
    return $out;
}
...
function subscriptionUserSnapshot(array $user): array {
    ...
    return [
        ...
        'account'=>[
            ...
            'plan'=>authPlan($user),
            'trial'=>authTrialStatus($user),
            'billing_plan'=>(string)($user['plan'] ?? 'free'),
            'billing_expires_at'=>(string)($user['plan_expires_at'] ?? ''),

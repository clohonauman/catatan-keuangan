...
function backupReferencedPhotoNames($d,$inbox=[]){
    $names=[];
    ...
    foreach((array)($inbox['items']??[]) as $row){$f=basename((string)($row['attachment']['file']??''));if($f!=='')$names[$f]=true;}
    return array_keys($names);
}

if($_SERVER['REQUEST_METHOD']==='GET'){
    $user=authCurrentUser();$d=financeReadData();$inboxExport=transactionInboxExportCurrentUser();$photos=[];$photoBytes=0;$skipped=[];$dir=DATA_DIR.'/receipts/user_'.(int)$user['id'];
    foreach(backupReferencedPhotoNames($d,$inboxExport) as $name){
    ...
    $payload=['format'=>'pencatatan-keuangan-backup','version'=>6,'username'=>$user['username']??'user','exported_at'=>date('c'),'data'=>$d,'assistant_learning'=>adaptiveExportCurrentUser(),'transaction_inbox'=>$inboxExport,'photos'=>$photos,'photos_skipped'=>$skipped];
...
try{
    ...
    // Backup v6 ikut membawa learning pribadi, Transaction Inbox, dan foto nota draft Inbox. Backup v1-v5 tetap kompatibel
    ...
    if(isset($payload['transaction_inbox']) && is_array($payload['transaction_inbox'])) transactionInboxImportCurrentUser($payload['transaction_inbox'],true);

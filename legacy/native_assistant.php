...
function categoryFor(string $chunk, string $type): string {
    ...
    $rules = $type==='income' ? [
        ...
        'Transfer'=>['transfer','dikirim','kiriman','masuk','pemasukan','pendapatan','terima','diterima','dapat','dapet','dikasih','diberi','kasih uang','beri uang'],
...
function typeFor(string $chunk): string {
    ...
    // Pemberian uang kepada user = pemasukan. Pola ini sengaja diperiksa
    // sebelum kata umum agar "Utari kasih uang tunai 50rb" tidak dianggap
    // pengeluaran. Sebaliknya "saya kasih Utari 50rb" tetap pengeluaran.
    if(preg_match('/\b(?:saya|aku|kita)\s+(?:kasih|beri|berikan|ngasih|kasi)\b/u',$c)) return 'expense';
    if(preg_match('/\b(?:dikasih|diberi|dapat|dapet|menerima|terima|diterima)\s+(?:uang|dana|cash|tunai)\b/u',$c)) return 'income';
    if(preg_match('/\b(?:kasih|beri|berikan|ngasih|kasi)\s+(?:saya|aku|kita)\s+(?:uang|dana|cash|tunai)\b/u',$c)) return 'income';
    if(preg_match('/^(?!\s*(?:saya|aku|kita)\b).{1,60}\b(?:kasih|beri|berikan|ngasih|kasi)\s+(?:uang|dana|cash|tunai)\b/u',$c)) return 'income';
    if(preg_match('/^(?!\s*(?:saya|aku|kita)\b).{1,60}\b(?:kasih|beri|berikan|ngasih|kasi)\s+(?:rp\s*)?\d[\d.,]*(?:\s*(?:rb|ribu|k|jt|juta))?\b/u',$c)) return 'income';
    if(preg_match('/^(?!\s*(?:saya|aku|kita)\b).{1,60}\b(?:transfer|kirim)\s+(?:ke\s+)?(?:saya|aku|kita)\b/u',$c)) return 'income';
    if(preg_match('/\b(?:uang|dana|cash|tunai)\s+(?:dari|masuk dari)\s+[^,.;]+/u',$c)) return 'income';
    ...
    foreach([
        ...
        'bunga','bunga tabungan','cashback','refund','pengembalian dana','setoran masuk'
    ] as $w) {
...
function amountRegex(): string {
    // Nominal tidak boleh menempel pada huruf/angka lain. Ini mencegah
    // angka di dalam plat/kode seperti DL2755CD dianggap sebagai uang.
    return '/(?<![a-z0-9])(?:rp\s*)?(?:\d{1,3}(?:[\.,]\d{3})+|\d+(?:[\.,]\d+)?)(?:\s*(?:juta|jt|ribu|rb|k))?(?![a-z0-9])/iu';
}

/**
 * Rentang angka yang jelas bukan nominal uang.
 * Disimpan sebagai byte offset karena PREG_OFFSET_CAPTURE juga menggunakan
 * byte offset. Tujuannya menjaga tanggal, jam, plat nomor, dan kode umum
 * agar tidak ikut dipilih sebagai nominal transaksi.
 */
function transactionProtectedNumberRanges(string $text): array {
    $ranges=[];
    $patterns=[
        // Tanggal: 21/09/2026, 21-09-26.
        '/\b\d{1,2}[\/-]\d{1,2}(?:[\/-]\d{2,4})?\b/u',
        // Jam: 08:30, 08:30:15.
        '/\b\d{1,2}:\d{2}(?::\d{2})?\b/u',
        // Plat Indonesia compact / berspasi: DL2755CD, DL 2755 CD, B 1234 ABC.
        '/\b[a-z]{1,2}\s*\d{1,4}\s*[a-z]{1,3}\b/iu',
    ];
    foreach($patterns as $pattern){
        if(!preg_match_all($pattern,$text,$m,PREG_OFFSET_CAPTURE))continue;
        foreach($m[0] as $hit){
            $raw=(string)$hit[0];$start=(int)$hit[1];
...
function transactionOffsetProtected(int $start,int $end,array $ranges): bool {
    foreach($ranges as $r){
        if($start < (int)$r[1] && $end > (int)$r[0]) return true;
    }
    return false;
}

/**
 * Kandidat nominal dengan skor konteks.
 * Prioritas: Rp/unit uang > pemisah ribuan > integer biasa.
 * Angka yang berada di tanggal/jam/plat/kode/rekening/referensi diabaikan.
 */
function transactionAmountCandidates(string $text,bool $allowSmallPlain=true): array {
    $text=normalizeChatMessage($text);
    if($text==='')return [];
    if(!preg_match_all(amountRegex(),$text,$m,PREG_OFFSET_CAPTURE))return [];
    $protected=transactionProtectedNumberRanges($text);
    $rows=[];
    foreach($m[0] as $hit){
        $raw=trim((string)$hit[0]);$offset=(int)$hit[1];$end=$offset+strlen((string)$hit[0]);
        if($raw==='' || transactionOffsetProtected($offset,$end,$protected))continue;
        $amount=parseAmount($raw);if($amount<=0)continue;

        $before=strtolower(substr($text,max(0,$offset-42),min(42,$offset)));
        $after=strtolower(substr($text,$end,24));
        $hasUnit=(bool)preg_match('/(?:^|[^a-z])(?:rp\s*)?\d[\d\.,]*\s*(?:juta|jt|ribu|rb|k)\b|\brp\s*\d/iu',$raw);
        $hasRp=(bool)preg_match('/\brp\s*\d/iu',$raw);
        $hasThousands=(bool)preg_match('/\d{1,3}(?:[\.,]\d{3})+/u',$raw);
        $digits=preg_replace('/\D/u','',$raw);
        $plain=!$hasUnit&&!$hasRp&&!$hasThousands;

        // Persentase bukan nominal transaksi.
        if(preg_match('/^\s*%/u',$after))continue;

        // Label identitas/kode tepat sebelum angka.
        if(preg_match('/(?:\b(?:no|nomor|nopol|plat|polisi|kode|ref|referensi|invoice|inv|rekening|rek|nik|nisn|hp|wa|telepon|telp|id)\s*(?:[:#.-]\s*)?)$/iu',$before))continue;

        // Tahun dan angka tanggal sederhana tidak dipakai sebagai nominal.
        if($plain && $amount>=1900 && $amount<=2100 && preg_match('/\b(?:tahun|thn)\s*$/u',$before))continue;
        if($plain && $amount<=31 && preg_match('/\b(?:tanggal|tgl)\s*$/u',$before))continue;

        // Angka panjang setelah kata rekening/kode sering merupakan identifier.
        if($plain && strlen($digits)>=7 && preg_match('/\b(?:rekening|rek|kode|ref|referensi|invoice|nik|nisn|hp|wa|telepon|telp)\b[^\n]{0,18}$/u',$before))continue;

        if($plain && !$allowSmallPlain && $amount<1000)continue;

        $score=40;
        if($hasRp||$hasUnit)$score=120;
        elseif($hasThousands)$score=105;
        elseif($amount>=1000)$score=75;

        // Kata nominal/uang/transaksi dekat kandidat memperkuat keyakinan.
        if(preg_match('/\b(?:nominal|seharga|harga|bayar|beli|masuk|keluar|pemasukan|pengeluaran|gaji|bonus|bunga|bbm|bensin|makan|transfer|tunai|cash)\b[^\n]{0,18}$/u',$before))$score+=12;
        $rows[]=['raw'=>$raw,'amount'=>$amount,'offset'=>$offset,'score'=>$score];
    }
    usort($rows,function($a,$b){
        $cmp=(int)$b['score']<=>(int)$a['score'];
        return $cmp!==0?$cmp:((int)$a['offset']<=>(int)$b['offset']);
    });
    return $rows;
}

function transactionAmountFromText(string $text,bool $allowSmallPlain=true): int {
    $rows=transactionAmountCandidates($text,$allowSmallPlain);
    return $rows?(int)$rows[0]['amount']:0;
}

/**
 * Pernyataan pencatatan transaksi harus dipisahkan dari follow-up analitik.
 * Contoh "masuk bunga tabungan seabank 9" adalah transaksi baru meskipun
 * menyebut nama dompet yang juga bisa muncul pada pertanyaan sebelumnya.
 */
function looksLikeTransactionStatement(string $message): bool {
    $t=norm(normalizeChatMessage($message));
    if($t==='')return false;
    if(smartIsQuestion($t))return false;
    if(transactionAmountFromText($t,true)<=0)return false;
    return (bool)preg_match('/\b(?:catat|catatkan|beli|bayar|dibayar|masuk|pemasukan|pendapatan|terima|diterima|dapat|gaji|bonus|thr|insentif|bunga|cashback|refund|jual|penjualan|bbm|bensin|pertamax|pertalite|makan|belanja|parkir|transfer|tarik tunai|setor|top\s*up|pengeluaran)\b/u',$t);
}
...
function extractTransactions(string $message): array {
    ...
    foreach($chunks as $chunk) {
        // Pilih kandidat uang terbaik, bukan sekadar angka pertama. Dengan ini
        // "BBM Pertamax DL2755CD 50rb" tetap membaca Rp50.000, sedangkan 2755
        // dikenali sebagai bagian plat kendaraan.
        $amount = transactionAmountFromText($chunk,true);
    ...
    if($saved){
        ...
        foreach($saved as $x) {
            ...
            if($type==='transfer'){
                ...
                $fromName=trim((string)($from['name']??'Dompet asal'));
                $toName=trim((string)($to['name']??'Dompet tujuan'));
                $parts[]='Transfer '.rupiah($x['amount']).' dari '.$fromName.' ke '.$toName;
                continue;
            }
            $walletLabel = walletLabelForTransaction($x);
            $direction = $type === 'income' ? ' ke ' : ' dari ';
            $parts[] = ($type==='expense'?'Pengeluaran':'Pemasukan').' '.rupiah($x['amount']).' ('.$x['category'].')'.($walletLabel!=='' ? $direction.$walletLabel : '');
            if (!empty($x['wallet_id'])) $walletIds[(int)$x['wallet_id']] = true;
        }

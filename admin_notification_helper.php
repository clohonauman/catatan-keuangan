...
function adminNotificationAppUrl(): string {
    ...
    return 'https://charlie-finance.rf.gd/';
}
...
function adminNotifySubscriptionEvent(array $order, string $event): void {
    ...
    foreach ($admins as $admin) {
        ...
        try { adminNotificationCreate($adminId, 'premium_'.$event, $title, $message, $payload); }
        catch (Throwable $e) { error_log('Gagal membuat notifikasi aplikasi admin: '.$e->getMessage()); }
        try { addChatForUser($adminId, 'assistant', '🔔 '.$title."\n".$message); }
        catch (Throwable $e) { error_log('Gagal menambahkan chat notifikasi admin: '.$e->getMessage()); }

...
function emailNotifyRead(): array { return emailNotifyNormalizeData(DocumentRepository::get('email','notifications',emailNotifyDefaultData())); }

function emailNotifyMutate(callable $fn){ return DocumentRepository::mutate('email','notifications',emailNotifyDefaultData(),function(&$data)use($fn){$data=emailNotifyNormalizeData($data);return $fn($data);}); }
...
function emailNotifyAndroidDownloadPage(): string {
    $base=rtrim((string)(Yii::$app->params['appUrl']??''),'/'); return ($base!==''?$base:'').'/android-download.php';
}

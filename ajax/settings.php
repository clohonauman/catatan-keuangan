...
if (array_key_exists('wallet_initial_balances', $input)) {
    try {
        ...
        foreach($items as $item){
            ...
            if(!is_int($id)||$id<1||!is_int($value)||$value<0||$value>9007199254740991||!is_int($expected)||!is_int($reserved)||$reserved<0||$reserved>9007199254740991||!is_int($expectedReserved)||$expectedReserved<0||isset($changes[$id]))throw new InvalidArgumentException('ID atau nominal saldo awal/dana disisihkan tidak valid.');
            $changes[$id]=['value'=>$value,'expected'=>$expected,'reserved'=>$reserved,'expected_reserved'=>$expectedReserved];
        }
        ...
        financeMutate(function(&$d)use($changes,$premium){
            ...
            foreach($changes as $id=>$change){
                ...
                auditAdd($d,'initial_balance','wallet',(int)$id,$before,$d['wallets'][$index],true,'Saldo awal / dana disisihkan diubah');
            }

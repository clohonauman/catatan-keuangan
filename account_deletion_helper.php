...
function cfDeleteAccountVerifyAndDelete($username, $secret, $secretType='password') {
    ...
    @unlink(__DIR__.'/data/user_finance/user_'.$uid.'.json');
    cfRecursiveDelete(__DIR__.'/data/receipts/user_'.$uid);

    // Hapus invoice langganan dan bukti pembayaran milik user.
    ...
    $learningFile = __DIR__.'/data/assistant_learning.json';
    if (file_exists($learningFile)) {
        $fp = @fopen($learningFile, 'c+');
        if ($fp) {
            flock($fp, LOCK_EX);
            rewind($fp);
            $learning = json_decode((string)stream_get_contents($fp), true);
            if (is_array($learning) && isset($learning['rules']) && is_array($learning['rules'])) {
                ...
                fwrite($fp, json_encode($learning, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE));
                fflush($fp);
            }

...
define('AUTH_FILE', __DIR__ . '/data/users.json');
...
function authEnsureData()
{
    ...
    if (!file_exists(AUTH_FILE)) {
        file_put_contents(AUTH_FILE, json_encode(authDefaultData(), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), LOCK_EX);
    }
}

function authReadData()
{
    authEnsureData();
    $raw = @file_get_contents(AUTH_FILE);
    $data = json_decode((string)$raw, true);
    return is_array($data) ? array_replace_recursive(authDefaultData(), $data) : authDefaultData();
}

function authMutateData($fn)
{
    authEnsureData();
    $fp = fopen(AUTH_FILE, 'c+');
    if (!$fp) throw new RuntimeException('Tidak dapat membuka file akun. Periksa permission folder data.');
    flock($fp, LOCK_EX);
    rewind($fp);
    $raw = stream_get_contents($fp);
    $data = json_decode($raw, true);
    if (!is_array($data)) $data = authDefaultData();
    else $data = array_replace_recursive(authDefaultData(), $data);
...
function authFindUserByUsername($username)
{
...
function authFindUserByEmail($email)
{
    $email = authNormalizeEmail($email);
    if ($email === '') return null;
    $data = authReadData();
    foreach ($data['users'] as $user) {
        if (authNormalizeEmail($user['email'] ?? '') === $email) return $user;
    }
    return null;
}
...
function authIssueDeviceToken($userId)
{
    ...
    authMutateData(function (&$data) use ($userId, $hash, $deviceId, $info, $now, $ip) {
        foreach ($data['users'] as &$user) {
...
function authResolveDevice()
{
    ...
    foreach ($data['users'] as $user) {
        foreach (($user['device_tokens'] ?? []) as $dt) {
            if (isset($dt['hash']) && hash_equals((string)$dt['hash'], $hash)) {
...
function authTouchCurrentDevice($userId, $force = false)
{
    ...
    $ip = authClientIp();
    $info = authDescribeDevice();
    authMutateData(function (&$data) use ($userId, $hash, $now, $ip, $info) {
        foreach ($data['users'] as &$user) {
...
function authRevokeDevice($userId, $deviceId)
{
    ...
    $currentHash = authCurrentDeviceHash();
    $result = authMutateData(function (&$data) use ($userId, $deviceId, $currentHash) {
        foreach ($data['users'] as &$user) {
...
function authRevokeOtherDevices($userId)
{
    ...
    $result = authMutateData(function (&$data) use ($userId, $currentHash) {
        foreach ($data['users'] as &$user) {
            if ((int)($user['id'] ?? 0) !== $userId) continue;
            $current = [];
            foreach (($user['device_tokens'] ?? []) as $dt) {
...
function authSetPin($userId, $pin)
{
    ...
    authMutateData(function (&$data) use ($userId, $pin) {
        foreach ($data['users'] as &$user) {
            if ((int)$user['id'] === (int)$userId) {
...
function authChangePassword($userId, $currentPassword, $newPassword)
{
    ...
    $result = authMutateData(function (&$data) use ($userId, $currentPassword, $newPassword) {
        foreach ($data['users'] as &$user) {
            if ((int)($user['id'] ?? 0) !== $userId) continue;
...
function authChangePin($userId, $currentPin, $newPin)
{
    ...
    $result = authMutateData(function (&$data) use ($userId, $currentPin, $newPin) {
        foreach ($data['users'] as &$user) {
            if ((int)($user['id'] ?? 0) !== $userId) continue;
...
function authLogout()
{
    ...
    if ($hash && $userId) {
        authMutateData(function (&$data) use ($userId, $hash) {
            foreach ($data['users'] as &$user) {
                if ((int)$user['id'] === (int)$userId) {
...
function authUpdateEmail($userId, $email)
{
    ...
    return authMutateData(function (&$data) use ($userId, $email) {
        foreach ($data['users'] as &$user) {
            if ((int)($user['id'] ?? 0) !== (int)$userId) continue;
...
function authRequestEmailVerification($userId)
{
    ...
    authMutateData(function (&$data) use ($userId, $hash, $expiresAt) {
        foreach ($data['users'] as &$u) {
...
function authVerifyEmailToken($userId, $code)
{
    ...
    $result = authMutateData(function (&$data) use ($userId, $hash) {
        foreach ($data['users'] as &$u) {
            if ((int)($u['id'] ?? 0) !== (int)$userId) continue;
...
{
    if (!$user) $user = authCurrentUser();
    if (!$user) return ['plan' => 'free', 'expires_at' => '', 'active' => false];
    $plan = (string)($user['plan'] ?? 'free');
    $exp = (string)($user['plan_expires_at'] ?? '');
    $active = $plan === 'premium' && ($exp === '' || $exp >= date('Y-m-d'));
    return ['plan' => $active ? 'premium' : 'free', 'expires_at' => $exp, 'active' => $active];
}
...
function authAdminSetUserPlan($userId, $plan, $expires = '')
{
    ...
    return authMutateData(function (&$data) use ($userId, $plan, $expires) {
